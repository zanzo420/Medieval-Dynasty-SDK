...
void UBTTask_WaitAnimation_C::AfterRead()
{
	UBTTask_BlueprintBase::AfterRead();

	READ_PTR_FULL(PawnReference, APawn);
	READ_PTR_FULL(WorkMontage, UAnimMontage);
	READ_PTR_FULL(NPC_Mesh, USkeletalMeshComponent);
	READ_PTR_FULL(InteractedFurniture, AActor);
}

void UBTTask_WaitAnimation_C::BeforeDelete()
{
	UBTTask_BlueprintBase::BeforeDelete();

	DELE_PTR_FULL(PawnReference);
	DELE_PTR_FULL(WorkMontage);
	DELE_PTR_FULL(NPC_Mesh);
	DELE_PTR_FULL(InteractedFurniture);
}
