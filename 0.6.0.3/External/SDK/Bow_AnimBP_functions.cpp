...
void UBow_AnimBP_C::AfterRead()
{
	UAnimInstance::AfterRead();

	READ_PTR_FULL(Item, ABP_HoldableItem_Bow_C);
	READ_PTR_FULL(ReloadMontage, UAnimMontage);
	READ_PTR_FULL(ShootMontage, UAnimMontage);
}

void UBow_AnimBP_C::BeforeDelete()
{
	UAnimInstance::BeforeDelete();

	DELE_PTR_FULL(Item);
	DELE_PTR_FULL(ReloadMontage);
	DELE_PTR_FULL(ShootMontage);
}
